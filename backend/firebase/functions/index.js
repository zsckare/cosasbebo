...
exports.createStripeCharge = functions.firestore.document('users/{userId}/charges/{id}').onCreate(async (snap, context) => {
    ...
    try {
        ...
        const receipt_email = val.receipt_email;

        const charge = { amount, currency, customer, description, receipt_email };
        if (val.source !== null) {
...
// exports.createStripeOrder = functions.firestore.document("users/{userId}/orders/{id}").onCreate(async(snap, context) => {
//     const val = snap.data();
//     try {
//         const snapshot = await admin.firestore().collection(`users`).doc(context.params.userId).get()
//         const snapval = snapshot.data()
